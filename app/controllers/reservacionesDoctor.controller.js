const Reservaciones = require("../models/reservacionesDoctor.model.js");
...
exports.findOne = (req, res) => {
  Reservaciones.findById(req.params.nro_reserva, (err, data) => {
    if (err) {
      if (err.kind === "not_found") {
        res.status(404).send({
          message: `No se encuentra el nro_reserva ${req.params.nro_reserva}.`,
        });
      } else {
        res.status(500).send({
          message:
            "Error al recuperar  el nro_reserva " + req.params.nro_reserva,
        });
      }
    } else res.send(data);
  });
    ...
    if (err) {
      if (err.kind === "not_found") {
        res.status(404).send({
          message:
            err.kind +
            ` No se encuentra el agendamiento con el codigo ${req.params.nro_reserva}.`,
        });
      } else {
        res.status(500).send({
          message:
