const Reservaciones = require("../models/reservaciones.model.js");
...
exports.findOne = (req, res) => {
    Reservaciones.findById(req.params.asegurado_cedula, (err, data) => {
      if (err) {
        if (err.kind === "not_found") {
          res.status(404).send({
            message: `No se encuentra asegurado con el ci ${req.params.asegurado_cedula}.`
          });
        } else {
          res.status(500).send({
            message: "Error al recuperar asegurado con el ci " + req.params.asegurado_cedula
          });
        }
      } else res.send(data);
    });
