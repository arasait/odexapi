    ...
    Consultorio.findById(req.params.consultorioId, (err, data) => {
      if (err) {
        if (err.kind === "not_found") {
          ...
          res.status(500).send({
            message: "Error al recuperar consultorio con el codigo " + req.params.consultorioId
          });
        }
      } else res.send(data);
    });
      ...
      (err, data) => {
        if (err) {
          if (err.kind === "not_found") {
            ...
            res.status(500).send({
              message: "Error al actualizar consultorio con el codigo " + req.params.consultorioId
            });
          }
        } else res.send(data);
      }
    ...
    Consultorio.remove(req.params.consultorioId, (err, data) => {
      if (err) {
        if (err.kind === "not_found") {
          ...
          res.status(500).send({
            message: "No se pudo borrar consultorio con el codigo " + req.params.consultorioId
          });
        }
...
exports.deleteAll = (req, res) => {
    Consultorio.removeAll((err, data) => {
      if (err)
        res.status(500).send({
          message:
            err.message || "Se produjo un error al eliminar todos los consultorios."
        });
      else res.send({ message: `Todos los consultorios fueron eliminados con éxito!` });
    });
  };
