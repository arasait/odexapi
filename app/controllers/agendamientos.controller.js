const Agendamientos = require("../models/agendamientos.model.js");
...
exports.create = (req, res) => {
  ...
  Agendamientos.create(reservacion, (err, data) => {
    if (err)
      res.status(500).send({
...
exports.findAll = (req, res) => {
  Agendamientos.getAll((err, data) => {
    if (err)
      res.status(500).send({
        message:
          err.message || "Se produjo un error al recuperar las agendamientos."
      });
    else res.send(data);
  });
};
...
exports.findOne = (req, res) => {
  Agendamientos.findById(req.params.asegurado_cedula, (err, data) => {
    if (err) {
...
exports.delete = (req, res) => {
  Agendamientos.remove(req.params.doctorId, (err, data) => {
    if (err) {
      if (err.kind === "not_found") {
        res.status(404).send({
          message: `No se encuentra doctor con el codigo ${req.params.doctorId}.`
...
exports.deleteAll = (req, res) => {
  Agendamientos.removeAll((err, data) => {
    if (err)
      res.status(500).send({
        message:
